...
export function ProductCard({ product, onEdit, onDelete }: Props) {
  ...
  return (
    ...
    >
      {/* glass card body */}
      <div className="rounded-[15px] bg-white/60 dark:bg-slate-900/65 backdrop-blur-xl overflow-hidden flex flex-col h-full">
        ...
        <div className="relative h-44 overflow-hidden bg-gradient-to-br from-slate-50/90 to-slate-100/70 dark:from-slate-800/70 dark:to-slate-900/80">
          ...
          {/* top shimmer line */}
          <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/80 dark:via-white/20 to-transparent z-10 pointer-events-none" />
            ...
            <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-1 px-2 z-10">
              {images.slice(0, 5).map((img, i) => (
                <button
                  ...
                  className={`w-7 h-7 shrink-0 overflow-hidden rounded-lg transition-all duration-150 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm shadow-soft ${
              ...
              {images.length > 5 && (
                <div className="w-7 h-7 shrink-0 rounded-lg bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm shadow-soft flex items-center justify-center text-[10px] text-fg-muted font-medium">
                  +{images.length - 5}
                </div>
              )}
            </div>
          ...
          <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10">
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-xl bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm shadow-soft text-fg-muted hover:text-primary transition-colors"
            >
            ...
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-xl bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm shadow-soft text-fg-muted hover:text-red-500 transition-colors"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* ── info area ── */}
        <div className="flex flex-col gap-1.5 flex-1 px-4 pt-3 pb-4 border-t border-white/40 dark:border-white/5">
          ...
          {product.listings.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-0.5">
              {product.listings.map(l => (
                <span
                  key={l.id}
                  className="text-[10px] text-fg-muted bg-white/50 dark:bg-white/5 border border-white/60 dark:border-white/10 px-1.5 py-0.5 rounded-full backdrop-blur-sm"
                >
                  {l.marketplace.name}
                </span>
              ))}
          ...
          <AnimatePresence>
            {confirming && (
              ...
              >
                <div className="mt-2 pt-2.5 border-t border-white/40 dark:border-white/10 flex items-center justify-between gap-2">
